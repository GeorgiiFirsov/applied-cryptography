...
\begin{enumerate}
    ...
    \item Оптимизация происходит за счет того, что при доказательстве можно пропустить проверку
        ...
        \begin{equation}
            \mathfrak{v}(n, k) = \lceil \log_2 n \rceil + 2 \cdot \mathfrak{l}(k) + \mathfrak{s}(k) + 1,
        \end{equation}
        где $\mathfrak{l}(k)$ --- количество <<обходных>> переходов, $\mathfrak{s}(k)$ --- количество
        <<обычных>> переходов. Дополнительная единица появляется в силу необходимости предоставить хэш
        заголовка генезис-блока ($H_0$).
        ...
        \begin{split}
            \mathfrak{v}(n, k) & = \lceil \log_2 n \rceil + 2 \cdot \frac{(d - 2)(d - 1)}{2} + 
                2 \cdot (d - 1) + 1 = \\
            & = \lceil \log_2 n \rceil + d (d - 1) + 1.
        \end{split}
        \end{equation}
        
        \Answer{$\mathfrak{v}(n, k) = \lceil \log_2 n \rceil + d(k) (d(k) - 1) + 1$, где 
            $d(k) = \log_2(k + 1)$.}
\end{enumerate}
